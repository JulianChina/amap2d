...
public class BusSegmentListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = View.inflate(mContext, com.run.sg.amap2d.R.layout.item_bus_segment, null);
			holder.parent = (RelativeLayout) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_item);
			holder.busLineName = (TextView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_line_name);
			holder.busDirIcon = (ImageView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_dir_icon);
			holder.busStationNum = (TextView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_station_num);
			holder.busExpandImage = (ImageView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_expand_image);
			holder.busDirUp = (ImageView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_dir_icon_up);
			holder.busDirDown = (ImageView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_dir_icon_down);
			holder.splitLine = (ImageView) convertView
					.findViewById(com.run.sg.amap2d.R.id.bus_seg_split_line);
			holder.expandContent = (LinearLayout) convertView
					.findViewById(com.run.sg.amap2d.R.id.expand_content);
		...
		if (position == 0) {
			holder.busDirIcon.setImageResource(com.run.sg.amap2d.R.drawable.dir_start);
			...
			holder.busDirIcon.setImageResource(com.run.sg.amap2d.R.drawable.dir_end);
			...
			if (item.isWalk() && item.getWalk() != null && item.getWalk().getDistance() > 0) {
				holder.busDirIcon.setImageResource(com.run.sg.amap2d.R.drawable.dir13);
				holder.busDirUp.setVisibility(View.VISIBLE);
				holder.busDirDown.setVisibility(View.VISIBLE);
				...
				holder.busDirIcon.setImageResource(com.run.sg.amap2d.R.drawable.dir14);
				holder.busDirUp.setVisibility(View.VISIBLE);
				holder.busDirDown.setVisibility(View.VISIBLE);
				...
				holder.busDirIcon.setImageResource(com.run.sg.amap2d.R.drawable.dir16);
				holder.busDirUp.setVisibility(View.VISIBLE);
				holder.busDirDown.setVisibility(View.VISIBLE);
	...
	private class ArrowClick implements OnClickListener {
			...
			if (mItem.isBus()) {
				if (mHolder.arrowExpend == false) {
					...
					mHolder.busExpandImage
							.setImageResource(com.run.sg.amap2d.R.drawable.up);
					...
					mHolder.busExpandImage
							.setImageResource(com.run.sg.amap2d.R.drawable.down);
		...
		private void addBusStation(BusStationItem station) {
			LinearLayout ll = (LinearLayout) View.inflate(mContext,
					com.run.sg.amap2d.R.layout.item_bus_segment_ex, null);
			TextView tv = (TextView) ll
					.findViewById(com.run.sg.amap2d.R.id.bus_line_station_name);
